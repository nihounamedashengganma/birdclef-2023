...
# 对训练样本做增强
def class_data_augmentation(df,aug_dic,num_classes,thr,seed):
    '''
...
if __name__ == '__main__':
    ...
    # 总共包括6种增强方式，对应的aug_type 依次为 1，2，3，4，5，6
    aug_dic = {'gaussian_noise':[0,0.2],
                'time_stretch':[0.2,0.4],
                'pitch_shift':[0.4,0.6],
                'shift':[0.6,0.8],
    ...
    # 构建增强数据集
    train_df_aug = class_data_augmentation(train_df,aug_dic,CONFIG.num_classes,50,CONFIG.random_seed)
    
    # 合并
    train_df =pd.concat([train_df,train_df_aug]).reset_index(drop=True)
